...
export default class TableList extends PureComponent {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'Order/setGetOlderOrderListbyFWRY',
      payload: params,
    });
  ...
  render() {
    ...
    if(wxbindUserlist){
        for (let i = 0; i < wxbindUserlist.list.length; i++) {
            ...
            children.push(<Option key={wxbindUserlist.list[i]['Admin_user_unionid']}>{"账号:"+wxbindUserlist.list[i]['Admin_user_num']+"| 姓名："+wxbindUserlist.list[i]['Admin_userinfo_name']}</Option>);
        }
    }
   // console.log(children)
    ...
    const columns = [
...

      {
        ...
        render: val => (
          <Fragment>
            <Button onClick={this.XJDWJDBTN.bind(this,val)}>
              接单
        </Button>
