...
import { Card, Button, Icon, List, Input, Form, Select, TextArea } from 'antd';
    ...
    const lb_sortNum_change = (e, q) => {
      ...
      list.map(item => {
        if (item.lb_id == e) {
          ...
          item.lb_sortNum_change = q.target.value;
        }
      });
      console.log(list);
    };
              ...
              item ? (
                <List.Item key={item.lb_id}>
                  <Card
                    ...
                    cover={<img alt="example" src={item.lb_img_url} style={{height:"300px"}} />}
                  >
                    <Card.Meta
